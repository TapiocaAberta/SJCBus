 ...
 *     });
     ...
     * @example
     <doc:example>
     <doc:source>
     <div ng-init="bus = [{name:'John', phone:'555-1276'},
     ...
     <tr ng-repeat="bus in bus | filter:searchText">
     <td>{{bus.name}}</td>
     <td>{{bus.phone}}</td>
     </tr>
     </table>
     ...
     <tr><th>Name</th><th>Phone</th></tr>
     <tr ng-repeat="busObj in bus | filter:search:strict">
     <td>{{busObj.name}}</td>
     <td>{{busObj.phone}}</td>
     </tr>
     </table>
     ...
     <doc:protractor>
     var expectBusNames = function(expectedNames, key) {
         element.all(by.repeater(key + ' in bus').column(key + '.name')).then(function(arr) {
     ...
     it('should search across all fields when filtering with a string', function() {
         ...
         expectBusNames(['Mary', 'Mike', 'Adam'], 'bus');

         searchText.clear();
         searchText.sendKeys('76');
         expectBusNames(['John', 'Julie'], 'bus');
       });
     ...
     it('should search in specific fields when filtering with a predicate object', function() {
         ...
         expectBusNames(['Mary', 'Mike', 'Julie', 'Juliette'], 'busObj');
       });
     ...
     * @example
     ...
     <tr ng-repeat="bus in bus | orderBy:predicate:reverse">
     <td>{{bus.name}}</td>
     <td>{{bus.phone}}</td>
     <td>{{bus.age}}</td>
     </tr>
 ...
 *                      'other': '{{person1}}, {{person2}} and {} other people are viewing.'}">
     ...
     * then uses `ngRepeat` to display every person:
     ...
     I have {{bus.length}} bus. They are:
     <input type="search" ng-model="q" placeholder="filter bus..." />
     <ul class="example-animate-container">
     <li class="animate-repeat" ng-repeat="bus in bus | filter:q">
     [{{$index + 1}}] {{bus.name}} who is {{bus.age}} years old.
     </li>
     </ul>
     </div>
     </file>
     ...
     <file name="protractorTest.js">
     var bus = element(by.css('.doc-example-live'))
     .element.all(by.repeater('bus in bus'));

     it('should render initial data set', function() {
        expect(bus.count()).toBe(10);
        expect(bus.get(0).getText()).toEqual('[1] John who is 25 years old.');
        expect(bus.get(1).getText()).toEqual('[2] Jessie who is 30 years old.');
        expect(bus.last().getText()).toEqual('[10] Samantha who is 60 years old.');
        expect(element(by.binding('bus.length')).getText())
            .toMatch("I have 10 bus. They are:");
      });
     ...
     it('should update repeater when filter predicate changes', function() {
         ...
         expect(bus.get(0).getText()).toEqual('[1] Mary who is 28 years old.');
         expect(bus.last().getText()).toEqual('[2] Samantha who is 60 years old.');
       });
     </file>
     </example>
     */
